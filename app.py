...
def render_sidebar(gen: SensorDataGenerator, pipeline: IndustrialMLPipeline):
    with st.sidebar:
        ...
        if train_btn or force_btn:
            # Remove old models if forcing retrain
            if force_btn:
                import glob
...
def render_fleet_overview(readings_df: pd.DataFrame):
    ...
    with col1:
        ...
        fig.update_layout(**plotly_layout, title="Fleet Health Scores", height=280,
                          yaxis=dict(**plotly_layout["yaxis"], range=[0, 115]))
...
def render_sensor_charts(readings_df: pd.DataFrame, selected_machine: str):
    ...
    # Normalize values 0-1 relative to fleet max
    sensor_values = []
    for s in sensors:
        ...
        for idx, (sensor, unit, color, min_v, max_v) in enumerate(gauge_data):
            ...
            with gauge_cols[idx % 3]:
                fig_g = go.Figure(go.Indicator(
                    ...
                    title={"text": sensor_display_name(sensor).split(" ", 1)[1] if " " in sensor_display_name(sensor) else sensor, "font": {"size": 9, "color": TEXT_COLOR}},
                    gauge={
...
def render_ai_insights(readings_df: pd.DataFrame, pipeline: IndustrialMLPipeline):
    ...
    if pipeline.failure_predictor.feature_importances:
        ...
        fig.update_layout(**plotly_layout, title="Top Predictive Features (Failure Model)",
                          height=350, yaxis=dict(**plotly_layout["yaxis"], autorange="reversed"))
        st.plotly_chart(fig, use_container_width=True)
...
def render_historical_analytics(hist_df: pd.DataFrame, selected_machine: str):
    ...
    for sensor, row, col, color in sensor_pairs:
    ...
    fig2.update_layout(**plotly_layout, title="Degradation Over Time — All Machines", height=350,
                       yaxis=dict(**plotly_layout["yaxis"], title="Degradation %"))
    st.plotly_chart(fig2, use_container_width=True)
...
def main():
    ...
    with tab2:
        ...
        st.dataframe(
            display_df.style.background_gradient(subset=["Health Score"], cmap="RdYlGn")
                            .format(precision=2),
